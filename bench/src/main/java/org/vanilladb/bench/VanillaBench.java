...
import org.vanilladb.bench.benchmarks.as2.As2Benchmark;
...
public class VanillaBench {
	...
	public void benchmark() {
		try {
			...
			emulators[0] = benchmarker.createRte(conn, statMgr, VanillaBenchParameters.RTE_SLEEP_TIME); // Reuse the
																										// connection
			for (int i = 1; i < emulators.length; i++)
				emulators[i] = benchmarker.createRte(getConnection(), statMgr, VanillaBenchParameters.RTE_SLEEP_TIME);
	...
	private StatisticMgr newStatisticMgr(Benchmark benchmarker) {
		...
		return new StatisticMgr(txnTypes, VanillaBenchParameters.REPORT_OUTPUT_DIRECTORY, reportPostfix,
